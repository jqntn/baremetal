...
for /r "thirdparty\printf" %%f in (*.c) do clang %CLANG_FLAGS% "%%f" -o "obj\%%~nf.o"
for /r "thirdparty\libc\src" %%f in (*.c) do clang %CLANG_FLAGS% -DDISABLE_UNIMPLEMENTED_LIBC_APIS "%%f" -o "obj\%%~nf.o"
for /r "thirdparty\gdtoa\src" %%f in (*.c) do clang %CLANG_FLAGS% -DNO_ERRNO -DIFNAN_CHECK -DGDTOA_NO_ASSERT -DNO_FENV_H -DNO_IEEE_Scale "%%f" -o "obj\%%~nf.o"
for /r "thirdparty\libmemory\src" %%f in (*.c) do clang %CLANG_FLAGS% "%%f" -o "obj\%%~nf.o"
